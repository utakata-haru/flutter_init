...
- ダッシュボードUI
  ...
  - 受け入れ基準: ルーチンが0件の場合は空表示を出し、エラー時はスナックバーで通知、設定画面への遷移アイコンを提供する
...
## 画面設計（概要）
- 主要画面: トップ（ステータス一覧）、ルーチン編集モーダル／画面、閾値設定ページ
- 画面遷移（テキスト／図示可）: トップ（/routine） → ルーチン編集（将来モーダル）、トップ（/routine） → 設定（/routine/settings）
